...
public class CarBehaviour : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (drone.position.x >= 22 && drone.position.x <= 59 && car.position.z < drone.position.z) { // проверяем на дороге ли дрон, и вообще находится ли машина до участка дороги, на котором находится дрон
            ...
            if (usedSpeed > speed * 0.1) usedSpeed = usedSpeed * 0.97f; // постепенно уменьшаем скорость, дабы не было моментальной остановки
            else usedSpeed = 0;
        } else if (usedSpeed < speed) usedSpeed = usedSpeed + speed * 0.03f; // постепенно повышаем скорость, опять же дабы не было моментального набора скорости
